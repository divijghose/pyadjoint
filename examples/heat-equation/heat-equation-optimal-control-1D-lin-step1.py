...
from pyadjoint import ParametrisedReducedFunctional, continue_annotation, ReducedFunctional
...
for m_i in m_list:
    ...
    m_i.interpolate(0.1) # A small Gaussian in the middle of the domain as an initial guess for the control.
...
solver_heat = LinearVariationalSolver(problem)


def loss_functional(mi, t_current, t_window):
...
# )
    return assemble(inner(mi, mi)*dx)
...
def time_hop_loop(controls, t_init, J):
    ...
    for m_i in controls:
        ...
        J+= loss_functional(m_i, t_current, t_window)
...
while t_actual < T:
    ...
    if window_num == 0:
        ...
        continue_annotation()
        J = loss_functional(m_list[0], t_actual, 0.0) 
        # Jhat = ReducedFunctional(J, [Control(m_list[0]), Control(u_init)], derivative_components=[0]) # We only optimize over the first control in the window, and keep the rest fixed as the initial guess. We also need to specify that u_init is a parameter, since it changes in each window.
        Jhat = ParametrisedReducedFunctional(J, [Control(m_list[0])], parameters=u_init)
        Jhat = Jhat._reduced_functional
        print(Jhat.controls)
        pause_annotation()
        PETSc.Sys.Print("visualising")
        tape = get_working_tape()
        tape.visualise_pdf("tape_visualisation_lin_1D_prf.pdf")
        solver = set_TAO_solver(Jhat)
        m_opt = get_optimal_control(solver)
        print(m_opt[0].dat.data)
        exit()
...
final_l2_error = l2_errors[-1] if l2_errors else norm(u_desired - u_new)
