...
from pyadjoint import ParametrisedReducedFunctional, pause_annotation
...
window_size = opts.getInt("--window-size", default=1) # Number of time steps in each window
...
def loss_functional(t_current, t_window, mi):
...
def time_hop_loop(controls, t_init, J):
    ...
    for m_i in controls:
        ...
        J+= loss_functional(t_current, t_window, m_i)
    return J
...
while t_actual < T:
    ...
    if window_num == 0:
        ...
        for m_i in m_list:
            print(m_i)
        J = loss_functional(0,0,m_list[0]) # This is needed to get the correct visualisation of the tape, since the tape is currently only visualised up to the point where the derivative is called.
        Jhat = ParametrisedReducedFunctional(J, [Control(m_i) for m_i in m_list], u_init)
        pause_annotation()
        PETSc.Sys.Print("visualising")
        tape = get_working_tape()
        tape.visualise_pdf("tape_visualisation.pdf")
        Jhat.derivative() 
        exit(0)# This is needed to get the correct visualisation of the tape, since the tape is currently only visualised up to the point where the derivative is called.
        # else:
        ...
        u_init.assign(u_new)
        window_num += 1
        m_list[:-window_step] = (m_opt[window_step:])# use assign
        for i in range(window_step):
